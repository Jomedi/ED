...
int recursiva(const BinTree<string>& t, map<string,int> &m) {
   if (t.left().empty() && t.right().empty()) {
        if (esLetra(t.root()[0]))
            return m[t.root()]; // O(num_variables)
        else 
            return stoi(t.root());
    }
    else {
        if (t.root()[0] == '*') 
            return recursiva(t.left(), m) * recursiva(t.right(), m);
        else if (t.root()[0] == '+') 
            return recursiva(t.left(), m) + recursiva(t.right(), m);
        else if (t.root()[0] == '-')
            return recursiva(t.left(), m) - recursiva(t.right(), m);
...
// Coste total = O(n-1) + 2*O(num_variables) siendo n el número de elementos del árbol y num_variables el número de variables que leemos de la entrada
bool tratar_caso() {
    int num_variables;
    cin >> num_variables;
    ...
    for (int i = 0; i < num_variables; i++) {
        t = parse(cin);
        recursiva(t.left(), m); //O(num_variables) + O(1)
        m[t.left().root()] = recursiva(t.right(), m); // O(num_variables) + O(n-2)
    }
